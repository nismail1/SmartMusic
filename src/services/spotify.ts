...
/** Same host pattern as tracks: GET /v1/artists/{id} on server for genres (browser catalog calls often 403). */
function resolveSpotifyArtistsProxyUrl(): string {
  const explicit = (import.meta.env.VITE_SPOTIFY_ARTISTS_PROXY_URL ?? "").trim();
  if (explicit) return explicit;
  const rec = (import.meta.env.VITE_RECOMMENDATIONS_ENDPOINT ?? "").trim();
  if (rec.includes("getRecommendations")) return rec.replace("getRecommendations", "getSpotifyArtistsByIds");
  return "";
}
...
function normalizeTrack(item: any): SpotifyTrack {
  const spotifyArtistIds: string[] = Array.isArray(item?.artists)
    ? item.artists.map((a: any) => (a && a.id ? String(a.id) : "")).filter(Boolean)
    : [];
  return {
    id: item.id,
    name: item.name,
    artists: Array.isArray(item.artists) ? item.artists.map((a: any) => a.name) : [],
    spotifyArtistIds: spotifyArtistIds.length ? spotifyArtistIds : undefined,
    uri: item.uri ?? "",
...
export function spotifyArtistDetailUrl(artistId: string): string {
  ...
  if (!id) throw new Error("Missing Spotify artist id");
  return `${SPOTIFY_ARTISTS_URL}/${encodeURIComponent(id)}`;
}

/** Normalize single-artist JSON from `GET /v1/artists/{id}`. Exported for tests. */
export function parseSpotifyArtistDetailJson(body: unknown, requestedArtistId: string): { id: string; genres: string[] } {
  const a = body as { id?: string; genres?: unknown };
  const id =
    a?.id != null && String(a.id).trim() ? String(a.id) : requestedArtistId.trim();
  const genres = Array.isArray(a?.genres)
    ? (a.genres as unknown[]).map((g) => String(g).trim()).filter(Boolean)
    : [];
  return { id, genres };
}

const ARTIST_DETAIL_CONCURRENCY = 8;

/** DEV-only: see whether Spotify returned genre strings (proxy or direct). */
function logDevArtistGenreSummary(
  source: "proxy" | "browser",
  requestedIds: string[],
  map: Map<string, string[]>
) {
  if (!import.meta.env.DEV) return;
  const rows = [...map.entries()].slice(0, 12).map(([id, genres]) => ({
    artistId: id,
  ...
  console.info(`[SmartMusic] Spotify artist genres (${source})`, {
    path:
      source === "proxy"
        ? "POST getSpotifyArtistsByIds → GET /v1/artists/{id} on server"
        : "browser GET /v1/artists/{id} + client token",
    idsRequested: requestedIds.length,
    artistsResolved: map.size,
    totalGenreStringsReturned: totalTags,
    perArtist: rows
  });
}
...
export const spotifyService = {
  ...
  async listCurrentUserPlaylists(accessToken: string): Promise<SpotifyUserPlaylist[]> {
    ...
    const task = (async () => {
      ...
      while (nextUrl) {
        /** 502 is usually an upstream/gateway fault; retrying often multiplies errors. Only retry 429 and 503/504 (limited). */
        const maxAttempts = 3;
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
          ...
          if (response.status === 502) {
            setTransientSpotifyFailureCooldown(cooldownKey, 12_000);
            break;
          }
          const transient53x = response.status === 503 || response.status === 504;
          if (transient53x && attempt < 2) {
            import.meta.env.DEV && fetch("http://127.0.0.1:7701/ingest/35369d23-7f37-4585-ac9a-076a3915746b", {
              method: "POST",
              headers: { "Content-Type": "application/json", "X-Debug-Session-Id": "658713" },
              body: JSON.stringify({
                ...
                message: "transient spotify error, retrying once",
                data: { status: response.status, attempt, nextUrl: nextUrl.slice(0, 60) },
                timestamp: Date.now()
              })
            }).catch(() => {});
            const backoff = Math.min(8_000, 900 * 2 ** (attempt - 1) + Math.floor(Math.random() * 400));
            await sleep(backoff);
            continue;
          }
          break;
